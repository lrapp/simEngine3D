...
r_start.shape=(3,1)
p_start.shape=(4,1)
X[0].q[0:3]=r_start #set initial position from HW7 
X[0].q[3:]=p_start #set initial position from HW7 

#%%
#check initial conditions
tol=1e-5
check_phi(X,constraint_list,0,tol)
PHI_P=1/2*(X[0].q[3:].T @ X[0].q[3:]) - 1/2
if PHI_P > tol:
...
G_dot=build_G(X[0].p_dot)
tau_hat=8*np.dot(np.dot(np.dot(G_dot.T,J_bar),G_dot),X[0].q[3:])
gamma_p=-2*np.dot(X[0].p_dot.T,X[0].p_dot) #slide 473
...
for ii in range(1,len(time_list)):
    ...
    while abs(error) > tol and count < 150:
        ...
        X_n[0].q[0:3]=r
        X_n[0].q[3:]=p
        X_n[0].A_rotation=build_A(X_n[0].q[3:])
        X_n[0].r_dot=r_d
        X_n[0].p_dot=p_d
        X_n[0].r_d_dot=z[0:3]
        X_n[0].p_d_dot=z[3:7]
        ...
        phi_q=calc_partials(X_n,constraint_list)            
